...
public class DeliveryRequestRepository implements CrudRepository<Long, DeliveryRequest>{
    ...
    public List<DeliveryRequest> readAll() {
        return jdbcTemplate.query(
                new PrepareStatementCreatorWithScrolledResultSet("SELECT delivery_request.id AS 'request_id', " +
                        "delivery_request.manager_id, \n" +
                        "user.first_name AS 'manager_first_name',\n" +
                        "user.last_name AS 'manager_last_name', delivery_request.courier_id, \n" +
                        "delivery_request.order_id, \n" +
                        "delivery_request.client_wish_description, delivery_request.creation_date, \n" +
